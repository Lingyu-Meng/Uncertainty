...
ggsave(file.path(save_path, "fig1_panel_B.png"), panel_B, width = 7, height = 7)

# Panel C, psychometric curve for directed exploration, uncertainty-dependent random exploration, and uncertainty-independent random exploration
## Directed exploration
V <- seq(-6, 6, 0.01)
no_dir   <- pnorm(V + 0 + V/ 1.72) # 1.72 is the standard deviation of the population (TU)
more_dir <- pnorm(V + 1 + V/ 1.72)
data_dir <- data.frame(V, no_dir, more_dir) %>% 
...
UDR_ex <- ggplot(data_UDR, aes(x = V, y = P, linetype = UDR, colour = uncertainty)) +
  ...
  labs(x = "Expected value difference V(1) - V(2)",
       ...
       linetype = "Uncertainty-dependent random exploration",
       colour = "Uncertainty") +
  scale_linetype_manual(values = c("solid", "dotdash")) +
  theme_cowplot() +
  scale_x_continuous(breaks = seq(-6, 6, 2)) +
...
UIR_ex <- ggplot(data_UIR, aes(x = V, y = P, linetype = UIR, colour = uncertainty)) +
  ...
  labs(x = "Expected value difference V(1) - V(2)",
       y = "Choice Probability P(1)",
       linetype = "Uncertainty-independent random exploration",
       colour = "Uncertainty") +
  scale_linetype_manual(values = c("solid", "dotdash")) +
  theme_cowplot() +
  scale_x_continuous(breaks = seq(-6, 6, 2)) +
