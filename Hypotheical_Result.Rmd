...
As previous study found that the relative risk in RR condition will be close to 0, I supposed there will be a different intercept between SR and RR condition. The TU is set to the real value of variance in the population. Based on the definition of TU: $TU=\sqrt{(\sigma_1^2+\sigma_2^2)}$, in our experiment, the TU in RR is actually smaller than in SR condition. The TU is SR is 
...
Notice: the sign of w3 is changed to negative.

The hybride model is a function with 3 variables, the w3 is actually a slop of the totel uncertainty (TU). By fixed the TU, the comparison of w3 will be anti-intuitive. A higher w3 will lead to a steep slop of value. Therefore, for convenience, I changed the sign of w3.
...
```{r save figures}
ggsave2("H4.png", H4, width = 6, height = 4)
ggsave2("H4_w3.png", H4_w3, width = 6, height = 4)
