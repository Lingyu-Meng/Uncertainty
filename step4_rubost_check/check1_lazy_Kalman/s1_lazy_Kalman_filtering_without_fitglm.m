...
function [nLL, results, eta, output] = KFL(parameters, input)
% parameters:  1 x 4 vector; contains eta, beta1, beta2, beta3
...
% probability of chosing left (1)
P = normcdf(beta1 * V + beta2 * RU + beta3 * VTU);
% negative LL as objective function
...
for n = 1:max(data.ID) % individual level
    ...
    initialPara = [1, 0, 0, 0];  
        % Set standard KF to a reasonable initial guess
    ...
    objFunHandle = @(par) KFL(par, data_sub);

    % Call fmincon to minimize the negative log-likelihood
    [para, negLogLikelihoodOpt] = fmincon(objFunHandle, initialPara, [], [], [], [], lb, ub, [], options);

    % Display results
    etalist(n) = para(1);
    % data(data.ID == n, :) = data_sub;
end
etalist(etalist < 0.99, :)
% cd("output/")
